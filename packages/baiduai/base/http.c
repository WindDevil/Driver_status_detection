...
int get_ai_result(const char *uri, const char *post_data, int post_data_size)
{
    ...
    do
    {
        ...
        if (RT_NULL == parse)
        {
           LOG_E("Fail to parse json about %s\n", "result");
        }

        user_list = cJSON_GetObjectItem(parse,"user_list");
        if (RT_NULL == user_list)
        {
           LOG_E("Fail to parse json about %s\n", "user_list");
        }

        user_list = cJSON_GetArrayItem(user_list,0);
        if (RT_NULL == user_list)
        {
           LOG_E("Fail to parse json about %s\n", "user_list");
        }

        user_id = cJSON_GetObjectItem(user_list,"user_id");
        if (RT_NULL == user_id)
        {
           LOG_E("Fail to parse json about %s\n", "user_id");
        }
        else
        {
           ...
           strcpy(result_Face,user_id->valuestring);
        }
